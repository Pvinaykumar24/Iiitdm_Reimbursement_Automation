    ...
    <>
      ...
      {pending.length > 0 && (
        <div className="alert alert-warning" style={{ marginBottom: 16 }}>
          ...
          <span style={{ marginLeft: 8, color: '#744FC6', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => navigate('/dean/pending')}>Review now →</span>
        </div>
      )}
      <div className="card">
        <div className="card-header">Claims Awaiting Approval (oldest first)</div>
        {pending.length === 0
          ? <div className="empty-state"><i className="ti ti-check" />No claims pending final approval. All done!</div>
