...
export default function DeanPendingClaims() {
  const [claims, setClaims] = useState([]);
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  useEffect(() => {
    claimsApi.getPendingDean().then(r => setClaims(r.data)).finally(() => setLoading(false));
  }, []);

  return (
    <>
      <h1 className="page-title">Claims Pending Approval</h1>
      <div className="card">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p style={{ fontWeight: 600, fontSize: 15 }}>No claims pending your approval. All done!</p>
          </div>
        ) : (
              ...
              {claims.map(c => (
                <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/dean/claims/${c.id}`)}>
                  ...
                  <td onClick={e => e.stopPropagation()} style={{ textAlign: 'right', paddingRight: 24 }}>
                    <button className="btn btn-primary btn-sm" style={{ height: 32, padding: '0 16px' }} onClick={() => navigate(`/dean/claims/${c.id}`)}>
                      Review
                    </button>
                  </td>
                </tr>
