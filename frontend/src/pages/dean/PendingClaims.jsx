...
export default function PendingClaims() {
  ...
  return (
    <>
      <h1 className="page-title">Pending claims</h1>
      <div className="card">
        {loading
          ? <div style={{ padding: 32, textAlign: 'center' }}><div className="spinner" style={{ margin: '0 auto' }} /></div>
          : claims.length === 0
            ? <div className="empty-state"><i className="ti ti-check" />No pending claims. All done!</div>
                ...
                <tbody>
                  {claims.map(c => (
                    <tr key={c.id}>
                      ...
                      <td><button className="btn btn-primary btn-sm" onClick={() => navigate(`/dean/claims/${c.id}`)}>Review</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
