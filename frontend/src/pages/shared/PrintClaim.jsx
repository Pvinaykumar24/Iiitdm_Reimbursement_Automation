...
export default function PrintClaim() {
  ...
  useEffect(() => {
    // Add screen preview helper style overrides on mount
  ...
  useEffect(() => {
    if (claim) {
      // Safe timeout to allow layout rendering and logo SVGs to fully paint
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', fontFamily: 'system-ui' }}>
        ...
        <div>Preparing print document...</div>
      </div>
    );
  }
  ...
  if (error || !claim) {
    return (
      <div style={{ padding: 24, textAlign: 'center', fontFamily: 'system-ui', color: '#c53030' }}>
        <h2>Error</h2>
  ...
  items.forEach(it => {
    ...
    const isClassified = it.sric_cgst !== null && it.sric_cgst !== undefined;
    const cgst = isClassified ? parseFloat(it.sric_cgst) : parseFloat(it.cgst_amount || 0);
    const sgst = isClassified ? parseFloat(it.sric_sgst) : parseFloat(it.sgst_amount || 0);
    const igst = isClassified ? parseFloat(it.sric_igst) : parseFloat(it.igst_amount || 0);
    const other = isClassified ? parseFloat(it.sric_other_charges) : parseFloat(it.other_charges || 0);
  ...
  return (
    <>
      ...
      <div className="print-control-bar">
        <span style={{ fontWeight: '600', fontSize: '14px' }}>Reimbursement Form Print Preview</span>
      ...
      <div className="print-container">
        ...
        <div className="print-header">
          {/* Official IIITDM Kancheepuram Crest Logo Image */}
          <img 
            src="/logo.png" 
            alt="IIITDM Logo" 
            style={{ width: '80px', height: '80px', marginRight: '15px', flexShrink: 0 }} 
          />
        ...
        <div className="print-title-box">
          REIMBURSEMENT FORM PRINCIPLE INVESTIGATORS -PROJECT
        </div>
        ...
        <table className="print-meta-table">
          <tbody>
            <tr>
              ...
              <td className="meta-val"><strong>{claim.claim_no || 'Draft'}</strong></td>
            ...
            <tr>
              ...
              <td className="meta-val"><strong>{claim.project_no}</strong></td>
            </tr>
            <tr>
              <td className="meta-label">Purpose of Expenditure:</td>
              <td className="meta-val" colSpan="3">{claim.purpose}</td>
            </tr>
          </tbody>
        </table>
        ...
        {invoicesList.map((inv, idx) => (
          <div key={idx} className="print-invoice-section">
            {/* Boxed Vendor & Bill Metadata */}
            <div className="print-invoice-meta-box" style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px' }}>
                <div>Vendor Name: <strong>{inv.vendor_name}</strong></div>
                <div style={{ textAlign: 'right' }}>Invoice No: <strong>{inv.bill_no}</strong></div>
                <div>Vendor GSTIN: <strong>{inv.gstin_vendor || '—'}</strong></div>
                <div style={{ textAlign: 'right' }}>Invoice Date: <strong>{new Date(inv.bill_date).toLocaleDateString('en-IN')}</strong></div>
              </div>
            </div>
            ...
            <table className="print-items-table">
              <thead>
                <tr>
                  <th style={{ width: '5%' }}>#</th>
                  <th style={{ width: isFaculty ? '65%' : '45%' }}>Description</th>
                  {!isFaculty && <th style={{ width: '20%' }}>Budget Head</th>}
                  <th style={{ width: '10%', textAlign: 'right' }}>Qty</th>
                  <th style={{ width: '10%', textAlign: 'right' }}>Unit Price</th>
                  <th style={{ width: '10%', textAlign: 'right' }}>Base Amt (₹)</th>
                </tr>
              </thead>
              <tbody>
                {inv.products.map((p, pIdx) => (
                  <tr key={pIdx}>
                    ...
                    <td>{p.description}</td>
                    {!isFaculty && <td><span className="print-head-badge">{p.budget_head || 'Pending Classification'}</span></td>}
                ...
                {/* Sub-total (Base Amount) Row */}
                <tr className="invoice-summary-row" style={{ background: '#f7fafc', fontWeight: 'bold' }}>
                  <td colSpan={isFaculty ? 4 : 5} style={{ textAlign: 'right' }}>Sub-total (Base Amount):</td>
                  <td style={{ textAlign: 'right' }}>₹{inv.base_total.toFixed(2)}</td>
                </tr>
                {/* Tax & Extra Rows */}
                {inv.cgst_total > 0 && (
                  <tr className="invoice-summary-row" style={{ background: '#fff' }}>
                    <td colSpan={isFaculty ? 4 : 5} style={{ textAlign: 'right', fontWeight: '500' }}>CGST:</td>
                    <td style={{ textAlign: 'right' }}>₹{inv.cgst_total.toFixed(2)}</td>
                  </tr>
                )}
                {/* SGST row */}
                {inv.sgst_total > 0 && (
                  <tr className="invoice-summary-row" style={{ background: '#fff' }}>
                    <td colSpan={isFaculty ? 4 : 5} style={{ textAlign: 'right', fontWeight: '500' }}>SGST:</td>
                    <td style={{ textAlign: 'right' }}>₹{inv.sgst_total.toFixed(2)}</td>
                  </tr>
                )}
                {/* IGST row */}
                {inv.igst_total > 0 && (
                  <tr className="invoice-summary-row" style={{ background: '#fff' }}>
                    <td colSpan={isFaculty ? 4 : 5} style={{ textAlign: 'right', fontWeight: '500' }}>IGST:</td>
                    <td style={{ textAlign: 'right' }}>₹{inv.igst_total.toFixed(2)}</td>
                  </tr>
                )}
                {/* Other Charges row */}
                {inv.other_total > 0 && (
                  <tr className="invoice-summary-row" style={{ background: '#fff' }}>
                    <td colSpan={isFaculty ? 4 : 5} style={{ textAlign: 'right', fontWeight: '500' }}>Other Charges / Cost:</td>
                    <td style={{ textAlign: 'right' }}>₹{inv.other_total.toFixed(2)}</td>
                  </tr>
                )}
                ...
                <tr className="invoice-summary-row" style={{ background: '#edf2f7', fontWeight: 'bold', borderTop: '1.5px solid #000' }}>
                  <td colSpan={isFaculty ? 4 : 5} style={{ textAlign: 'right', textTransform: 'uppercase' }}>Invoice Grand Total:</td>
        ...
        <h4 className="section-subtitle">{isFaculty ? '2. Certification & Submission' : '3. Certifications & Approval Workflow'}</h4>
        ...
        {/* Accounts Voucher block */}
        {!isFaculty && (
          <div className="accounts-voucher-box">
            <div className="voucher-header">For Use In Accounts Section (Dean Office)</div>
            <div className="voucher-body">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
                <div>
                  <label style={{ fontSize: '10px', color: '#4a5568', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Voucher Number:</label>
                  <div style={{ borderBottom: '1px dotted #718096', height: '20px' }} />
                </div>
                <div>
                  <label style={{ fontSize: '10px', color: '#4a5568', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Passed Amount (₹):</label>
                  <div style={{ borderBottom: '1px dotted #718096', height: '20px' }} />
                </div>
                <div>
                  <label style={{ fontSize: '10px', color: '#4a5568', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Accounts Officer Sign:</label>
                  <div style={{ borderBottom: '1px dotted #718096', height: '20px' }} />
                </div>
              </div>
            </div>
        ...
        <style>{`
          ...
          /* Screen overrides when viewing the print page */
          body.print-route, body.print-route #root {
            height: auto !important;
            min-height: 100vh !important;
            overflow: auto !important;
            background: #f0f4f8; /* A nice light background for preview */
          }
