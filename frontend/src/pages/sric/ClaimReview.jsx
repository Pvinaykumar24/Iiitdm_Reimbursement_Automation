  ...
  return (
    <>
        ...
        {claim.status !== 'DRAFT' && claim.status !== 'SRIC_PENDING' && (
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 12 }}>
            {claim.status === 'DEAN_PENDING' && (
              ...
              >
                <i className="ti ti-edit" style={{ marginRight: 8, fontSize: 16 }} />Edit Segregation
              </button>
            )}
            <button 
              className="btn btn-ghost" 
              onClick={() => window.open(`/claims/${claim.id}/print?role=sric`, '_blank')} 
              style={{ background: '#fff', height: 44, padding: '0 20px' }}
            >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24 }}>
            <div>
              ...
              <div style={{ fontWeight: 700, marginTop: 6, fontSize: 16 }}>
                <span style={{ color: '#744FC6', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => navigate(`/sric/faculty/${claim.faculty_id}`)}>
                  {claim.faculty_name}
                </span>
              </div>
      ...
      {claim.items && claim.items.length > 0 && (
        <div className="card" style={{ marginBottom: 28 }}>
          <div className="card-header" style={{ fontSize: 16, padding: '16px 32px' }}><i className="ti ti-chart-pie" style={{ marginRight: 8, color: '#744FC6' }} />Budget Segregation Summary</div>
          <div className="card-body" style={{ display: 'flex', gap: 16, flexWrap: 'wrap', padding: '24px 32px' }}>
            {Object.entries(budgetHeadSummaries).map(([bh, amt]) => (
              <div key={bh} style={{ background: '#f8fafc', padding: '14px 20px', borderRadius: 10, minWidth: 180, border: '1.5px solid #e1e0de', boxShadow: '0 1px 3px rgba(0,0,0,0.01)' }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 800, color: '#16a34a', marginTop: 6, fontFamily: "'Outfit', sans-serif" }}>₹{amt.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {isPending ? (
        <div className="card" style={{ border: '1.5px solid #bfdbfe' }}>
          <div className="card-header" style={{ fontSize: 16, padding: '16px 32px', background: '#eff6ff', borderBottom: '1px solid #bfdbfe', color: '#1e3a8a' }}>
            <i className="ti ti-stamp" style={{ marginRight: 8 }} /> SRIC Cell Verification & Action
          </div>
          <div className="card-body" style={{ padding: '32px' }}>
            <div className="form-group" style={{ marginBottom: 24 }}>
              <label className="form-label" style={{ fontSize: 15, marginBottom: 8 }}>Verification Remarks <span style={{ color: '#dc2626' }}>(Required if returning or rejecting)</span></label>
              <textarea rows={4} value={remarks} onChange={e => setRemarks(e.target.value)}
                placeholder="Enter detailed verification notes, corrections needed, or approval justification here..."
                style={{ padding: '12px 16px', fontSize: 15 }} />
            </div>
            {validationErrors.length > 0 && (
              <div style={{ color: '#dc2626', fontSize: 15, marginBottom: 20, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, background: '#fef2f2', padding: '12px 16px', borderRadius: 8, borderLeft: '4px solid #ef4444' }}>
                <i className="ti ti-alert-triangle" style={{ fontSize: 18 }} />
                <span>Segregation totals do not match the entered invoices. Correct the classification details above to verify this claim.</span>
              </div>
            )}
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 12 }}>
              ...
              <button className="btn btn-success" style={{ height: 48, padding: '0 28px' }} onClick={() => decide('APPROVED')} disabled={submitting || validationErrors.length > 0}>
                <i className="ti ti-check" style={{ marginRight: 8, fontSize: 16 }} />{submitting ? 'Processing...' : 'Verify & Forward to Dean'}
              </button>
            </div>
          </div>
        </div>
        ...
        <>
          <div className="card">
            <div className="card-header" style={{ fontSize: 16, padding: '16px 32px' }}><i className="ti ti-file-text" style={{ marginRight: 8, color: '#744FC6' }} />SRIC Verification Decision Details</div>
            <div className="card-body" style={{ padding: '32px' }}>
              <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', fontSize: 15 }}>
                <div>
                  <div style={{ fontSize: 13, color: '#64748b', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Status</div>
                  <span className={`badge ${claim.status === 'SRIC_REJECTED' ? 'badge-rejected' : 'badge-approved'}`} style={{ padding: '6px 14px', fontSize: 13 }}>
                    <i className={`ti ${claim.status === 'SRIC_REJECTED' ? 'ti-circle-x' : 'ti-circle-check'}`} style={{ marginRight: 6, fontSize: 14 }} />
                    {claim.status === 'SRIC_REJECTED' ? 'Rejected by SRIC' : 'Verified by SRIC'}
                  </span>
                </div>
                {sricApproval && (
                  <>
                    <div>
                      <div style={{ fontSize: 13, color: '#64748b', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Processed By</div>
                      <div style={{ fontWeight: 700, color: '#4C4C9D' }}>{sricApproval.actor_name}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 13, color: '#64748b', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Date Processed</div>
              ...
              {sricApproval?.remarks && (
                <div style={{ marginTop: 20, padding: '16px 20px', background: '#f8fafc', borderRadius: 8, border: '1.5px solid #d5d3d0', fontSize: 15, lineHeight: 1.5, color: '#334155' }}>
                  <div style={{ fontSize: 12, color: '#64748b', marginBottom: 6, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Verification Remarks</div>
                  "{sricApproval.remarks}"
                </div>
              )}
            </div>
                ...
                <tbody>
                  {inv.products.map((p, pIdx) => (
                    ...
                    >
                      <td>{pIdx + 1}</td>
                      <td>
                        <div style={{ fontWeight: 600, color: '#4C4C9D' }}>{p.description}</div>
                      </td>
                        ...
                        {isPending ? (
                          ...
                          p.budget_head ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                              <span className="badge badge-approved" style={{ width: 'fit-content' }}>{p.budget_head}</span>
                              {(parseFloat(p.sric_cgst) > 0 || parseFloat(p.sric_sgst) > 0 || parseFloat(p.sric_igst) > 0 || parseFloat(p.sric_other_charges) > 0) && (
                                <div style={{ fontSize: 12, color: '#475569', marginTop: 4, lineHeight: 1.5, background: '#f8fafc', padding: '6px 10px', borderRadius: 6, border: '1px solid #e1e0de' }}>
                                  <strong style={{ color: '#744FC6' }}>Segregated Tax:</strong>
